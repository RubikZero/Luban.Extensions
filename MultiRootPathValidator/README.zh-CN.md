...
- 面向 .NET 8 构建、版本 4.1.0 及之后的 Luban
- 构建本扩展需要 .NET 8 SDK
...
[共用 CI action](../.github/actions/build-against-luban/action.yml) 会把它部署进支持范围内的每一个 Luban 发行版，并针对 fixture 里带 `path` 标签的字段跑两次：一次把「不可能含该资源的根目录」排在「含该资源的根目录」**之前**（必须通过），一次只给那个不可能的根目录（必须被报出）。两次检查读的都是 Luban 的输出而非退出码，因为校验失败只会记日志、不会让运行失败。
